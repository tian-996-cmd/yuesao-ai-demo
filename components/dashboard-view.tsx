...
export function DashboardView({customers,nurses,onOpenCustomer,onCreateCustomer,onMatching,onSchedule,onParse}:{customers:Customer[];nurses:MaternityNurse[];onOpenCustomer:(id:string)=>void;onCreateCustomer:()=>void;onMatching:(id?:string)=>void;onSchedule:()=>void;onParse:(id:string)=>void}){
  ...
  return <div className="page-stack owner-dashboard">
    <div className="page-heading"><div><div className="eyebrow">老板工作台</div><h1>今日业务概览</h1><p>客户推进、人员流转与档期风险集中查看。</p></div><div className="heading-actions"><Button variant="outline" onClick={()=>onParse('wang')}><ScanText/>AI 整理需求</Button><Button className="primary-button" onClick={onCreateCustomer}><Plus/>新建客户</Button></div></div>
    ...
    <section className="flow-board"><header><div><h2>未来人员流转</h2><p>未来 15 天上户与下户安排</p></div><Button variant="ghost" onClick={onSchedule}>查看档期中心<ArrowRight/></Button></header><div>{flow.length?flow.map(x=><button key={x.slot.id} onClick={onSchedule}><span className="mini-nurse-avatar">{x.nurse.name[0]}</span><span><strong>{x.nurse.name}</strong><small>{x.slot.status} · {formatShort(x.slot.start)}–{formatShort(x.slot.end)}</small></span></button>):<p className="empty-inline">未来15天暂无人员流转</p>}</div></section>
  </div>;
}
