...
import { ArrowRight, Plus, RotateCcw, Search, UsersRound } from 'lucide-react';
...
export function CustomersView({ customers, onOpen, onCreate, onReset }: { customers:Customer[]; onOpen:(id:string)=>void; onCreate:()=>void; onReset:()=>void }) {
  ...
  return <div className="page-stack">
    ...
    <section className="surface customer-surface">
      <div className="customer-toolbar"><div className="search-box"><Search /><Input value={query} onChange={e=>setQuery(e.target.value)} placeholder="搜索客户姓名" aria-label="搜索客户姓名" /></div><div className="filter-tabs">{filters.map(item=><button className={filter===item?'active':''} key={item} onClick={()=>setFilter(item)}>{item}</button>)}</div><Button variant="ghost" className="reset-button" onClick={onReset}><RotateCcw />恢复演示数据</Button></div>
